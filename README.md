...
Many options for controlling the plots generated by `plotFit` have been introduced. The default x-axis label provides more useful information. `plotOneFit` has been removed since `plotFit` now provides the ability to control graphic elements. Related changes have been made to `getTiter` and `getEC63`. 
