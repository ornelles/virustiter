...
#' \code{\link{propagate()}} function. 
#' 
#' @param nmask A segmented nuclear mask such as generated by
#'   \code{nucMask()}.
#' @param cmask An optional \code{Image} object or an array containing a binary
#'   mask of cell regions that can be segmented. If this value is \code{NULL},
#'   the nuclear mask (\code{nmask}) will be expanded for Voronoi segmentation. 
#' @param brush Size of the brush size to expand the nuclear mask as an odd
#'   number of pixels. If this value is \code{NULL}, the mean value of the major
#'   axis of all nuclei will be used. 
#' @param lambda A numeric value used by \code{propagate()} determining the trade-off
#'   between the Euclidean distance in the image plane and the contribution of the
#'   gradient. See \code{\link{propagate()}} for details.
#'
#' @details
#'
...
#' disc-shaped brush of size equal to the mean semi-major axis of the nuclei.
#' If \code{cmask} is non-\code{NULL}, this is used as a binary mask to limit 
#' the Voronoi segmentation based on seeds provided by \code{nmask}. This
#' mask can be created as a binary mask from a non-specific cytoplasmic
#' stain such as actin or a diffuse membrane stain.
#'
#' To create a cytoplasmic mask that excludes the nucleus, simply subtract
#' the nuclear mask from the cell mask:
#'
