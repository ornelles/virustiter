...
# confirm availability of moi variable (change name for fitting purposes...)
	...
	if (!is.numeric(res[[param]]))
		stop("unable to use non-numeric values for '", param, "'")
...
# determine how proceed from 'by'
	...
	fmList <- list()
	if (by == "sequential") {
		form <- as.formula(paste0("cbind(pos, neg) ~ offset(log(", param, "))"))
		fm <- try(glm(form, data = res,
				subset = res[[param]] > 0, family = binomial("cloglog")))
		...
		for (i in levels(as.factor(res[[by]]))) {
			...
			fm <- try(glm(form, family = binomial("cloglog"), data = res,
					subset = res[[by]] == i & res[[param]] > 0), silent = TRUE)
			if (class(fm)[1] == "try-error")
				fmList[[i]] <- NULL
			else
				fmList[[i]] <- fm
